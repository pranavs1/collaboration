...
private SessionFactory sessionFactory;
	public List<User> getAllSuggestedUsers(String email) {
		...
		String queryString="select * from user_detail where email " //S in (A - (B U C))
				+ "in "
				+ "(select email from user_detail where email!=:e3 "
				+ "minus "
				+ "(select fromid_email from friend_s190124 where toid_email=:e1 "
				+ "union "
				+ "select toid_email from friend_s190124 where fromid_email=:e2))";
